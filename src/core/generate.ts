...
/**
 * Rewrites `<shared>/tools/index.ts`, the list of every tool.
 *
 * The file is derived from the `*Tool.ts` files present, never patched: a
 * generated list that is rebuilt from the directory cannot drift, and needs no
 * fragile text surgery. It is marked as generated so nobody edits it.
 */
export function writeToolRegistry(projectRoot: string, sharedToolsDirectory: string): void {
	const directory = resolveProjectPath(projectRoot, sharedToolsDirectory);
	const tools = readdirSync(directory)
		.map((file) => /^(.+Tool)\.ts$/.exec(file)?.[1])
		.filter((name): name is string => name !== undefined)
		.sort();

	const source = [
		"// Generated by `rowork make:tool`. Do not edit: this file is rewritten every time.",
		'import type { ToolDefinition } from "./ToolDefinition";',
		...tools.map((name) => `import { ${name} } from "./${name}";`),
		"",
	...
	writeFileSync(join(directory, "index.ts"), source, "utf8");
}
